...
# Load your OpenAI API key from env variable
...
def build_prompt(data: ChatbotInput) -> str:
    """
    Create a GPT prompt based on the study phase.
    """
    base = f"""
...
def chatbot_handler(data: ChatbotInput):
    ...
    # Simple heuristic to suggest moving on (can improve later)
    action = None
    if data.current_phase.phase == "flashcards" and "correct" in gpt_reply.lower():
        action = ActionSuggestion(type="next_phase", reason="Student seems to understand this flashcard.")
